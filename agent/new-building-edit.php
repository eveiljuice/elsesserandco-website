<?php
/**
 * Agent New Building Edit - Elsesser & Co.
 * Добавление/редактирование ЖК агентом
 */
...
if ($isEdit) {
    ...
    $stmt = $pdo->prepare("SELECT * FROM new_buildings WHERE id = ? AND (agent_id = ? OR ? IN (SELECT id FROM users WHERE role = 'admin'))");
    $stmt->execute([$buildingId, $userId, $userId]);
    $building = $stmt->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $agentId = $userId; // Агент автоматически привязывается к своим ЖК

<?php
/**
 * Developer Registration Handler - Elsesser & Co.
 * Обработка заявки на регистрацию застройщика
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_POST['csrf_token']) || !validateCSRFToken($_POST['csrf_token'])) {
        ...
        $completed_projects = filter_var($_POST['completed_projects'] ?? '', FILTER_VALIDATE_INT);
        ...
        $formData = compact(
            ...
            'description', 'specialization', 'years_on_market', 'completed_projects'
        );
        ...
        if (isset($_FILES['logo']) && $_FILES['logo']['error'] !== UPLOAD_ERR_NO_FILE) {
            if ($_FILES['logo']['error'] === UPLOAD_ERR_OK) {
                ...
                if (!in_array($mimeType, $allowedTypes)) {
                    $errors[] = "Логотип должен быть изображением (JPEG, PNG, GIF, WebP)";
                } elseif ($file['size'] > $maxSize) {
                    $errors[] = "Размер логотипа не должен превышать 5 МБ";
                } else {
        ...
        if (empty($errors)) {
            try {
                ...
                $stmt = $pdo->prepare("SELECT id, status FROM developer_applications WHERE inn = ?");
                $stmt->execute([$inn]);
                ...
                $stmt = $pdo->prepare("SELECT id FROM developer_applications WHERE email = ? AND status IN ('pending', 'reviewing')");
                $stmt->execute([$email]);
                if ($stmt->fetch()) {
                ...
                if (empty($errors)) {
                    ...
                    $stmt = $pdo->prepare("
                        INSERT INTO developer_applications (
                            company_name, legal_form, inn, ogrn, legal_address, actual_address,
                            contact_person, contact_position, email, phone, website,
                            description, specialization, years_on_market, completed_projects, logo, status
                        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')
                    ");
                    ...
                    $stmt->execute([
                        ...
                        $completed_projects !== false ? $completed_projects : null,
                        $logoPath
                    ]);
                    ...
                    // Логирование
                    error_log("New developer application: {$company_name} (INN: {$inn})");
                }
                
            } catch (PDOException $e) {
                error_log("Developer registration error: " . $e->getMessage());
